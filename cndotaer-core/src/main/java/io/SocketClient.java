...
/**
 * Created by cuiguiyang on 2017/3/12 16:16.
 * Desc
 */
public class SocketClient {
    ...
    public static void main(String[] args) {
        final String msg = "\n我是传奇"+
                ...
                "落霞与孤鹜齐飞，秋水共长天一色";
        final String tengwangge = "滕王阁序 王勃\n" +
        ...
        try {
            ...
            socket.connect(new InetSocketAddress(ConstantEnum.HOST, ConstantEnum.PORT));
            socket.getOutputStream().write(URLEncoder.encode(tengwangge, UTF8).getBytes(Charset.forName("UTF-8")));


            /*InputStream rece = socket.getInputStream();
            if (rece != null) {
                byte[] bytes = new byte[1024];
                int len;
                StringBuffer receive = new StringBuffer();
                while ((len = rece.read(bytes)) != -1) {
                    receive.append(URLDecoder.decode(new String(bytes, OFFSET, len), UTF8));
                }
                System.out.println("receive the answer msg: " + receive.toString());
            }*/
